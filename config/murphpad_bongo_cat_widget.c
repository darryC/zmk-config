...
#include "murphpad_bongo_cat_widget.h"
...
void state_widget_wpm(struct murphpad_bongo_cat_widget *widget, int wpm) {
    ...
    } else if (wpm < CONFIG_murphpad_BONGO_CAT_SLOW_LIMIT) {
...
int murphpad_bongo_cat_widget_init(struct zaphod_bongo_cat_widget *widget, lv_obj_t *parent) {
...
lv_obj_t *zaphod_bongo_cat_widget_obj(struct murphpad_bongo_cat_widget  *widget) {
    return widget->obj;
}

int wpm_status_listener(const zmk_event_t *eh) {
    struct murphpadd_bongo_cat_widget *widget;
...
ZMK_LISTENER(murphpad_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(murphpad_bongo_cat_widget, zmk_wpm_state_changed);
